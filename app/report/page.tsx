...
console.log(geminiApiKey, googleMapsApikey);
...
export default function ReportPage() {
  ...
  /*To get Recent Completed report*/
  ...
  //for waste verification process state idel|verified|success
  ...
  const [isIsSubmiitting, setIsSubmitting] = useState(false);
  //search the location
  ...
  //function for gemini ai verify the waste
  const handleVerify = async () => {
    ...
    try {
      ...
      const base64Data = await readFilesAsBase64(file);
      const imageParts = [
        {
          inlineData: {
      ...
      try {
        ...
        if (
          ...
          console.error("Failed to parse JSON response", parsedResult);
      ...
      console.error("Invalid varification response", error);
      setVerificationStatus("failure");
    }
  };
  //Submit button
  ...
  //check user authentication and fetch the recent report
  useEffect(() => {
    ...
    <div className="p-8 max-w-4xl mx-auto">
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
          ...
          <div>
            ...
            <input
              ...
              id="amount"
              name="amount"
              className="block  w-full text-sm rounded-xl border border-gray-300 px-4 py-2 focus:outline-none focus:ring-2 focus:ring-green-500 transition-all duration-300 bg-gray-100"
              readOnly
              onChange={handleInputChange}
        ...
        <Button
          ...
          disabled={isIsSubmiitting || verificationStatus !== "success"}
        >
          {isIsSubmiitting ? (
            <>
              {" "}
              <Loader calcMode="animted-spin -ml-1 mr-3 h-5 w-5 text-white" />
              submitting...
            </>
          ) : (
            "Submit Report"
